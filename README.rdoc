...
1. Create a rails application and check the results
    rhc app create -a someapp -t ruby-1.9
    rhc app show-app someapp
    
1. Add a mongoDB cartridge to your application
    rhc cartridge add -a someapp -c mongodb-2.2
   
1. Add this upstream repository. Note you may need to add a merge message.
    cd someapp
    git remote add upstream -m master git@github.com:alim/ignition.git
    git pull -s recursive -X theirs upstream master

1. Edit the config/application.yml to add your admin login
    cp config/application.yml.template config/application.yml

1. Edit the .gitignore file to comment out the line that ignores the application.yml file
    # /config/application.yml    

1. Edit the application.yml file to include an administrator login and email settings for your environments. The production one is shown, but you can add others for development and test.
    production:
...
1. Point your browser to the applicatin URL. You should now be able to the application using the email and password that you entered in the application.yml file.
